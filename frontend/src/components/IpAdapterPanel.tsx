  ...
  async function handleDownload() {
    ...
    try {
      await streamIpAdapterDownload(ev => {
        ...
        if (ev.type === 'error')    { setDlMsg(''); setDlError(ev.message ?? 'Download failed') }
      })
      ...
      {!downloaded && !downloading && (
        <div className="space-y-2">
          ...
          <button
            ...
            className="w-full py-2 rounded bg-accent text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            Download IP-Adapter weights
          </button>
          {dlError && (
            <p className="text-[11px] text-red-400 break-words">{dlError}</p>
          )}
          <div className="flex items-center gap-1">
            ...
            <span className="text-[11px] text-muted">What is IP-Adapter?</span>
          </div>
        </div>
      )}
      ...
      {downloading && (
        <div className="space-y-2">
          ...
          <div className="h-1 bg-border rounded-full overflow-hidden">
            <div className="h-full bg-accent transition-all duration-300" style={{ width: `${dlPct}%` }} />
          </div>
        </div>
      )}
            ...
            <div className="flex gap-2 flex-wrap">
              {slots.map(slot => (
                <div key={slot.slotId} className="flex flex-col items-center gap-1">
                  <div
                    className={`relative w-16 h-16 rounded border-2 overflow-hidden bg-surface ${dragOver === slot.slotId ? 'border-accent' : 'border-border'}`}
                  ...
                  />
                  <span className="text-[10px] text-muted">{slot.scale.toFixed(2)}</span>
                </div>
              ))}

              {slots.length < 3 && (
                <div
                  className={`w-16 h-16 rounded border-2 border-dashed flex items-center justify-center cursor-pointer transition-colors ${dragOver === -1 ? 'border-accent bg-accent/10' : 'border-border hover:border-accent'}`}
